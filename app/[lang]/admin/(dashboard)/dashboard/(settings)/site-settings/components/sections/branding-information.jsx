    ...
    const onSubmit = async formData => {
        ...
        try {
            ...
            if (themeRes && !themeRes.success) {
                toast.error(
                    typeof themeRes.error === 'string'
                        ? themeRes.error
                        : themeRes.error?.message || 'An error occurred'
                );
                return;
            }
            ...
            if (
                ...
                formData.logo.imageId
            ) {
                imagePayload.logo = formData.logo;
            }
            if (
                formData.favicon &&
                typeof formData.favicon === 'object' &&
                formData.favicon.imageId
            ) {
            ...
            if (Object.keys(imagePayload).length > 0) {
                ...
                if (infoRes && !infoRes.success) {
                    toast.error(
                        typeof infoRes.error === 'string'
                            ? infoRes.error
                            : infoRes.error?.message || 'An error occurred'
                    );
                    return;
                }
            }
                                ...
                                <div>
                                    ...
                                    <div className='grid grid-cols-1 gap-y-4'>
                                        ...
                                        <ColorPickerField
                                            ...
                                            control={methods.control}
                                            validationRules={
                                                validationRules?.secondaryColor
                                            }
                                            placeholder='#64748b'
                                        />
